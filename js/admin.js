...
function showAdminSection(sectionName) {
  ...
  switch (sectionName) {
    ...
    case 'gestion-usuarios-admin': renderGestionUsuariosAdmin(); break;
...
async function renderGestionUsuariosAdmin() {
  const container = document.getElementById('admin-sections');
  container.innerHTML = sectionPanelHTML(
    'Gestión de Usuarios',
    'Administrá datos y familiares de los trabajadores',
    `<div class="loading-center"><div class="spinner"></div></div>`
  );
  ...
  try {
    ...
    window._adminGestionUsuarios = data || [];
    const body = document.querySelector('#admin-sections .section-body');
    ...
    body.innerHTML = `
      <div class="search-bar" style="margin-bottom:16px">
        <svg viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" fill="currentColor"/></svg>
        <input type="text" id="admin-gestion-search" placeholder="Buscar por nombre, DNI o email..." oninput="filtrarAdminGestionUsuarios()" />
      </div>
      <div id="admin-gestion-lista">
        ${data.map(u => adminGestionUserCardHTML(u)).join('')}
      </div>
    `;
  } catch {
...
function filtrarAdminGestionUsuarios() {
  const q = document.getElementById('admin-gestion-search')?.value?.toLowerCase() || '';
  const filtered = (window._adminGestionUsuarios || []).filter(u =>
    (u.full_name || '').toLowerCase().includes(q) ||
    (u.email || '').toLowerCase().includes(q) ||
    (u.dni || '').includes(q)
  );
  const lista = document.getElementById('admin-gestion-lista');
  if (lista) lista.innerHTML = filtered.map(u => adminGestionUserCardHTML(u)).join('');
}

function adminGestionUserCardHTML(u) {
  const initials = (u.full_name || u.email || 'U').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  const isActive = u.active !== false;
  const familyCount = u.family_members?.[0]?.count || 0;

  return `
    <div class="gestion-card" style="${!isActive ? 'opacity:0.6' : ''}">
      <div class="gestion-card-top">
        ...
        <span class="status-badge ${isActive ? 'status-responded' : 'status-pending'}" style="font-size:11px;flex-shrink:0">
      ...
      <div style="margin:10px 0 12px">
        <button class="btn btn-secondary btn-sm" onclick="verFamiliaresAdmin('${u.id}', '${(u.full_name||'').replace(/'/g,'')}')">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;border-top:1px solid var(--gray-100);padding-top:12px">
        <button class="btn btn-outline btn-sm" onclick="abrirEditarUsuarioAdmin('${u.id}')">
        ...
        <button class="btn btn-sm ${isActive ? 'btn-danger' : 'btn-success'}" onclick="toggleActivarUsuarioAdmin('${u.id}', ${isActive})">
...
async function toggleActivarUsuarioAdmin(userId, currentlyActive) {
  const nuevoEstado = !currentlyActive;
  if (!confirm(`¿Querés ${nuevoEstado ? 'activar' : 'desactivar'} este usuario?`)) return;
  try {
    const sb = getSupabase();
    const { error } = await sb.from('profiles').update({ active: nuevoEstado }).eq('id', userId);
    if (error) throw error;
    const idx = (window._adminGestionUsuarios || []).findIndex(u => u.id === userId);
    if (idx !== -1) window._adminGestionUsuarios[idx].active = nuevoEstado;
    showToast(`Usuario ${nuevoEstado ? 'activado' : 'desactivado'}`, nuevoEstado ? 'success' : 'info');
    renderGestionUsuariosAdmin();
  } catch {
    showToast('Error al actualizar estado', 'error');
  }
}

async function verFamiliaresAdmin(userId, userName) {
  openModal(`
...
function abrirEditarUsuarioAdmin(userId) {
  const u = (window._adminGestionUsuarios || []).find(u => u.id === userId);
  if (!u) return;
  openModal(`
    <h2 class="modal-title">Editar Usuario</h2>
    <p class="modal-subtitle">${u.email}</p>
    <div id="edit-ua-error" class="alert alert-error hidden"></div>
    <div id="edit-ua-success" class="alert alert-success hidden"></div>
    <div class="form-group"><label>Nombre y Apellido</label><input type="text" id="ua-name" value="${u.full_name || ''}" /></div>
    <div class="profile-info-row">
      <div class="form-group"><label>DNI</label><input type="text" id="ua-dni" value="${u.dni || ''}" /></div>
      <div class="form-group"><label>Fecha de nacimiento</label><input type="date" id="ua-birth" value="${u.birthdate || ''}" /></div>
    </div>
    <div class="form-group"><label>Obra Social</label><input type="text" id="ua-os" value="${u.obra_social || ''}" /></div>
    <div class="profile-info-row">
      <div class="form-group"><label>Plan</label><input type="text" id="ua-plan" value="${u.plan || ''}" /></div>
...
async function guardarEdicionUsuarioAdmin(userId) {
  const errEl = document.getElementById('edit-ua-error');
  const sucEl = document.getElementById('edit-ua-success');
  const updates = {
    full_name: document.getElementById('ua-name')?.value?.trim(),
    dni: document.getElementById('ua-dni')?.value?.trim(),
    birthdate: document.getElementById('ua-birth')?.value || null,
    obra_social: document.getElementById('ua-os')?.value?.trim(),
  ...
  try {
    ...
    if (error) throw error;
    const idx = (window._adminGestionUsuarios || []).findIndex(u => u.id === userId);
    if (idx !== -1) window._adminGestionUsuarios[idx] = { ...window._adminGestionUsuarios[idx], ...updates };
    errEl.classList.add('hidden');
    showAlert(sucEl, '✓ Datos actualizados correctamente.');
    setTimeout(() => { closeModal(); renderGestionUsuariosAdmin(); }, 1200);
  } catch {
